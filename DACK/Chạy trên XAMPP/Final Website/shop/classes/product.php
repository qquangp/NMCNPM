...
<?php
	...
	class product
	{
		...
		public function insert_product($data,$files){
			...
			if($productName=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type=="" || $gender=="" || $file_name ==""){
				$alert = "<span class='error'>Fields must be not empty</span>";
				return $alert;
			}else{
				move_uploaded_file($file_temp,$uploaded_image);
				$query = "INSERT INTO tbl_product(productName,brandId,catId,product_desc,price,type,gender,image) VALUES('$productName','$brand','$category','$product_desc','$price','$type','$gender','$unique_image')";
		...
		public function update_product($data,$files,$id){
			...
			if($productName=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type==""|| $gender==""){
				$alert = "<span class='error'>Fields must be not empty</span>";
				return $alert;
			}else{
				if(!empty($file_name)){
		...
		// San pham noi bat
		public function getproduct_hottest(){
			$query = "SELECT * FROM tbl_product where type = '0' order by RAND() LIMIT 4 "; // lay ra 4 san pham
			$result = $this->db->select($query);
			return $result;
		}
        public function getproduct_gender(){
